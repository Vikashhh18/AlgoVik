  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      try {
        ...
        const res = await axios.get(`${baseUrl}/api/progress?sheetName=top50`, {
          headers: {
...
const toggleProblemStatus = async (problemId) => {
  ...
  return;
    ...
    const existing = useProgress.find(
      (entry) => entry.questionId === problemId && entry.sheetName === 'top50'
    );
  ...
  const isSolved = (id) => {
    return useProgress.some(
      (entry) => entry.questionId === id && entry.sheetName === 'top50' && entry.status === 'solved'
    );
  };

  const solvedCount = useProgress.filter(
    (entry) => entry.sheetName === 'top50' && entry.status === 'solved'
  ).length;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 space-y-4">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Most frequently asked coding questions in technical interviews
          </p>
