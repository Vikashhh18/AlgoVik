...
const Dashboard = () => {
        ...
        const sheetResults = await Promise.all(
          sheets.map(async (sheet) => {
            try {
              ...
              const progress = Math.round((solved / sheet.total) * 100);
  ...
  // Guard against division by zero for average progress
  const avgProgress =
    sheetProgress.length > 0
      ? Math.round(sheetProgress.reduce((acc, s) => acc + s.progress, 0) / sheetProgress.length)
      : 0;

  const totalQuestions = sheetProgress.reduce((acc, s) => acc + s.total, 0);
  const totalSolved = sheetProgress.reduce(
    (acc, s) => acc + Math.round((s.progress * s.total) / 100),
    0
  );
  const estHours = Math.round((totalQuestions - totalSolved) * 0.5);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="space-y-12">
          ...
          <section>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {sheetProgress.map((sheet, i) => (
                ...
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/0 to-indigo-500/0 group-hover:from-indigo-500/5 group-hover:to-indigo-500/5 dark:group-hover:from-indigo-500/10 dark:group-hover:to-indigo-500/10 transition-all duration-500" />
                  <div className="p-6">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: '📝', label: 'Total Questions', value: totalQuestions, bg: 'bg-blue-100 dark:bg-blue-900/30' },
              { icon: '✅', label: 'Questions Solved', value: totalSolved, bg: 'bg-green-100 dark:bg-green-900/30' },
              { icon: '📊', label: 'Average Progress', value: `${avgProgress}%`, bg: 'bg-purple-100 dark:bg-purple-900/30' },
              { icon: '⏳', label: 'Est. Time to Complete', value: `${estHours} hrs`, bg: 'bg-yellow-100 dark:bg-yellow-900/30' },
            ].map((stat, i) => (
